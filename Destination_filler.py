def DestinationFiller():
    ...
    # read data
    df = pd.read_csv("train_filled_Deck.csv")
    df = df.dropna()
    ...
    cat_cols = [col for col in (cat_col + num_but_cat) if col != "Destination"]
    ...
    X.drop(columns=["Deck_T"], inplace=True)
    ################################################################### ENCODER ##############################################################
    ################################################################### ENCODER ##############################################################
    print("Destination Train Filling working.....")
    ...
    original_df = pd.read_csv("train_filled_Deck.csv")

    missing_homeplanet_df = original_df[original_df['Destination'].isna()]
    ...
    missing_homeplanet_df.drop(["Destination"], axis=1, inplace=True)
    ...
    predictions_df = pd.DataFrame(predictions, columns=["Destination"])

    # Update the 'HomePlanet' column in missing_homeplanet_df with the predictions
    missing_homeplanet_df["Destination"] = predictions_df["Destination"]
    ...
    original_df_sorted = pd.concat([
        original_df[original_df['Destination'].isnull()],  # Rows with missing 'HomePlanet' values
        original_df.dropna(subset=['Destination'])        # Rows with non-missing 'HomePlanet' values
    ])
    ...
    original_df_sorted.loc[original_df_sorted['Destination'].isnull(), 'Destination'] = missing_homeplanet_df['HomePlanet']

    # Save the updated DataFrame to a new CSV file
    original_df_sorted.to_csv("train_filled_Destination.csv", index=False)
    print(f"Destination Train Filling DONE with %{accuracy} accuracy")
    ...
    # read data
    df = pd.read_csv("test_filled_Deck.csv")
    df = df.dropna()
    ...
    ################################################################### ENCODER ##############################################################
    ################################################################### ENCODER ##############################################################
    print("Destination Test Filling working.....")
    ...
    original_df = pd.read_csv("test_filled_Deck.csv")
    ...
    # Save the updated DataFrame to a new CSV file
    original_df_sorted.to_csv("test_filled_Destination.csv", index=False)
    print(f"Destination Test Filling DONE with %{accuracy} accuracy")
