def DeckFiller():
    ...
    # read data
    df = pd.read_csv("train_filled_VIP.csv")
    ...
    df.to_csv("train_deck_num_side.csv", index=False)

    df = pd.read_csv("train_deck_num_side.csv")
    df = df.dropna()
    ...
    df.drop(columns=["PassengerId","Name","Side","Num"],
            inplace=True)
    ...
    cat_cols = [col for col in (cat_col + num_but_cat) if col != "Deck"]
    ...
    print("Deck Train Filling working.....")
    ...
    original_df = pd.read_csv("train_deck_num_side.csv")

    missing_homeplanet_df = original_df
    missing_homeplanet_df.drop(columns=["Side"], inplace=True)
    missing_homeplanet_df.drop(columns=["Num"], inplace=True)
    ...
    missing_homeplanet_df.drop(columns=["PassengerId", "Name"], inplace=True)
    original_df = pd.read_csv("train_deck_num_side.csv")
    ...
    missing_homeplanet_df["Age"].fillna(value=selected_age, inplace=True)
    ...
    missing_homeplanet_df = missing_homeplanet_df[missing_homeplanet_df['Deck'].isna()]
    missing_homeplanet_df.drop(["Deck"], axis=1, inplace=True)
    ...
    predictions_df = pd.DataFrame(predictions, columns=["Deck"])

    # Update the 'HomePlanet' column in missing_homeplanet_df with the predictions
    missing_homeplanet_df["Deck"] = predictions_df["Deck"]
    ...
    original_df_sorted = pd.concat([
        original_df[original_df['Deck'].isnull()],
        original_df.dropna(subset=['Deck'])
    ])
    ...
    original_df_sorted.loc[original_df_sorted['Deck'].isnull(), 'Deck'] = missing_homeplanet_df['Deck']

    # Save the updated DataFrame to a new CSV file
    original_df_sorted.to_csv("train_filled_Deck.csv", index=False)
    print(f"Deck Train Filling DONE with %{accuracy} accuracy")
    ...
    warnings.filterwarnings("ignore")
    # read data
    df = pd.read_csv("test_filled_VIP.csv")
    ...
    df.to_csv("test_deck_num_side.csv", index=False)

    df = pd.read_csv("test_deck_num_side.csv")
    df = df.dropna()
    ...
    X.drop(columns=["pass_group_seat_8"], inplace=True)
    X.drop(columns=["pass_group_seat_6"], inplace=True)
    X.drop(columns=["pass_group_seat_4"], inplace=True)
    ...
    print("Deck Test Filler working.....")
    ...
    original_df = pd.read_csv("test_deck_num_side.csv")
    ...
    encoded_data_test = pd.DataFrame(encoded_data_test, columns=encoder.get_feature_names_out(columns_to_encode))
    missing_data_df = pd.concat([missing_homeplanet_df.drop(columns_to_encode, axis=1), encoded_data_test], axis=1)
    #missing_data_df.drop(columns=["CryoSleep_nan"], inplace=True)
    ...
    # Save the updated DataFrame to a new CSV file
    original_df_sorted.to_csv("test_filled_Deck.csv", index=False)
    print(f"Deck Test Filling DONE with %{accuracy} accuracy")
