def CryoSleepFiller():
    ...
    # read data
    df = pd.read_csv("train_filled_home.csv")
    df = df.dropna()
    ...
    cat_cols = [col for col in (cat_col + num_but_cat) if col != "CryoSleep"]
    ...
    ################################################################### ENCODER ##############################################################
    ################################################################### ENCODER ##############################################################
    print(f"CryoSleep Train Filling working.....")
    ...
    original_df = pd.read_csv("train_filled_home.csv")

    missing_homeplanet_df = original_df[original_df["CryoSleep"].isna()]
    ...
    missing_homeplanet_df.drop(["CryoSleep"], axis=1, inplace=True)
    ...
    missing_homeplanet_df['pass_group_seat_8'] = 0
    missing_homeplanet_df.reset_index(drop=True, inplace=True)
    missing_homeplanet_df[columns_to_encode] = missing_homeplanet_df[columns_to_encode].astype(str)
    ...
    predictions_df = pd.DataFrame(predictions, columns=["CryoSleep"])

    # Update the 'HomePlanet' column in missing_homeplanet_df with the predictions
    missing_homeplanet_df["CryoSleep"] = predictions_df["CryoSleep"]


    # Concatenate the DataFrame such that rows with missing 'HomePlanet' values are moved to the top
    original_df_sorted = pd.concat([
        original_df[original_df["CryoSleep"].isnull()],  # Rows with missing 'HomePlanet' values
        original_df.dropna(subset=["CryoSleep"])        # Rows with non-missing 'HomePlanet' values
    ])
    ...
    original_df_sorted.loc[original_df_sorted["CryoSleep"].isnull(), "CryoSleep"] = missing_homeplanet_df["CryoSleep"]

    # Save the updated DataFrame to a new CSV file
    original_df_sorted.to_csv("train_filled_cryo.csv", index=False)
    print(f"CryoSleep Train Data DONE with %{accuracy} accuracy")
    ...
    # read data
    df = pd.read_csv("test_filled_home.csv")
    df = df.dropna()
    ...
    original_df = pd.read_csv("test_filled_home.csv")
    ...
    # Save the updated DataFrame to a new CSV file
    original_df_sorted.to_csv("test_filled_cryo.csv", index=False)
    print(f"CryoSleep Test Data Filling DONE with %{accuracy} accuracy")
