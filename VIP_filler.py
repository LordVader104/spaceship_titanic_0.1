def VIPFiller():
    ...
    # read data
    df = pd.read_csv("train_filled_cryo.csv")
    df = df.dropna()
    ...
    cat_cols = [col for col in (cat_col + num_but_cat) if col != "VIP"]
    ...
    ################################################################### ENCODER ##############################################################
    ################################################################### ENCODER ##############################################################
    print("VIP Train Filling working.....")
    ...
    original_df = pd.read_csv("train_filled_cryo.csv")
    missing_homeplanet_df = original_df[original_df["VIP"].isna()]
    ...
    #percentage_CryoSleep = per_calculator(missing_homeplanet_df, "VIP", True)
    ...
    missing_homeplanet_df.drop(["VIP"], axis=1, inplace=True)
    ...
    predictions_df = pd.DataFrame(predictions, columns=["VIP"])

    # Update the 'HomePlanet' column in missing_homeplanet_df with the predictions
    missing_homeplanet_df["VIP"] = predictions_df["VIP"]
    ...
    original_df_sorted = pd.concat([
        original_df[original_df["VIP"].isnull()],  # Rows with missing 'HomePlanet' values
        original_df.dropna(subset=["VIP"])        # Rows with non-missing 'HomePlanet' values
    ])
    ...
    original_df_sorted.loc[original_df_sorted["VIP"].isnull(), "VIP"] = missing_homeplanet_df["VIP"]

    # Save the updated DataFrame to a new CSV file
    original_df_sorted.to_csv("test_filled_VIP.csv", index=False)
    print("VIP Train Data Filling DONE")
    ...
    # read data
    df = pd.read_csv("test_filled_cryo.csv")
    df = df.dropna()
    ...
    ################################################################### ENCODER ##############################################################
    ################################################################### ENCODER ##############################################################
    print("VIP Test Data Filling working.....")
    ...
    original_df = pd.read_csv("test_filled_cryo.csv")
    missing_homeplanet_df = original_df[original_df["VIP"].isna()]
    ...
    # Save the updated DataFrame to a new CSV file
    original_df_sorted.to_csv("test_filled_VIP.csv", index=False)
    print(f"VIP Test Data Filling DONE With %{accuracy} accuracy")
